...
function MasterItemSelect({ kind, selectedId, selectedName, onSelect }: { kind: string; selectedId: string | null; selectedName: string; onSelect: (item: SparkTargetMasterOption | null) => void }) {
  ...
  useEffect(() => { setItems([]); setElement(""); setCategory(""); setSeries(""); setSearched(false); }, [kind]);
  async function search() {
    const normalizedQuery = query.trim();
    if (!normalizedQuery) return;
    setLoading(true);
    setSearched(true);
    try { const result = await api.sparkTargetMasterOptions(kind, normalizedQuery, { element, category: kind === "summon" ? undefined : category, series: kind === "summon" ? series : undefined }); setItems(result.items); }
    catch { setItems([]); }
    finally { setLoading(false); }
  }
  return <div className="spark-master-setting"><span>対象</span><button className="secondary-button" onClick={() => setOpen(true)} type="button">対象設定</button><small>{selectedId ? `選択中：${selectedName}` : "未選択（名前の直接入力も可能）"}</small>{selectedId ? <button className="spark-master-clear" onClick={() => onSelect(null)} type="button">マスター選択を解除</button> : null}
    {open ? <div className="modal-backdrop spark-master-modal-backdrop" onMouseDown={() => setOpen(false)}><section aria-labelledby="spark-master-dialog-title" aria-modal="true" className="panel spark-dialog spark-master-dialog" onMouseDown={(event) => event.stopPropagation()} role="dialog"><header className="spark-dialog-header"><h2 id="spark-master-dialog-title">狙い目の対象を選択</h2><button aria-label="閉じる" className="icon-button" onClick={() => setOpen(false)} title="閉じる" type="button"><X size={19} /></button></header><div className="spark-dialog-body"><div className="spark-master-search"><label>テキスト検索<input autoFocus maxLength={100} onChange={(event) => { setQuery(event.target.value); setSearched(false); }} onKeyDown={(event) => { if (event.key === "Enter") { event.preventDefault(); void search(); } }} placeholder="名前・別名を入力" required type="search" value={query} /></label><label>属性<select onChange={(event) => setElement(event.target.value)} value={element}><option value="">すべて</option>{masterElements.map((value) => <option key={value} value={value}>{value}</option>)}</select></label>{kind === "summon" ? <label>シリーズ<select onChange={(event) => setSeries(event.target.value)} value={series}><option value="">すべて</option>{summonSeries.map((value) => <option key={value.value} value={value.value}>{value.label}</option>)}</select></label> : <label>区分<select onChange={(event) => setCategory(event.target.value)} value={category}><option value="">すべて</option>{masterCategories.map((value) => <option key={value} value={value}>{value}</option>)}</select></label>}<button className="primary-button" disabled={loading || !query.trim()} onClick={() => void search()} type="button">{loading ? "検索中…" : "検索"}</button></div><div aria-live="polite" className="spark-master-results">{items.map((item) => <button key={item.id} onClick={() => { onSelect(item); setOpen(false); }} type="button"><strong>{item.displayName ?? item.name}</strong><span>{[item.element, item.category].filter(Boolean).join("・")}</span></button>)}{!loading && !items.length ? <p className="empty-state">{searched ? "該当する対象はありません。" : "検索語を入力して検索してください。"}</p> : null}</div></div></section></div> : null}
  </div>;
}
