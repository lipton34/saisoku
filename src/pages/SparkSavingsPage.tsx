...
function MasterItemSelect({ kind, selectedId, onSelect }: { kind: string; selectedId: string | null; onSelect: (item: SparkTargetMasterOption | null) => void }) {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      setLoading(true);
      void api.sparkTargetMasterOptions(kind, query).then((result) => { if (active) setItems(result.items); }).catch(() => { if (active) setItems([]); }).finally(() => { if (active) setLoading(false); });
    }, 250);
    return () => { active = false; window.clearTimeout(timer); };
  }, [kind, query]);
  return <fieldset className="spark-master-select"><legend>マスターから検索（任意）</legend><input aria-label="マスターを検索" onChange={(event) => setQuery(event.target.value)} placeholder="名前・別名・限定区分で検索" type="search" value={query} /><select aria-label="マスター候補" disabled={loading || !items.length} onChange={(event) => { const item = items.find((candidate) => candidate.id === event.target.value); if (item) onSelect(item); }} value={selectedId ?? ""}><option value="">{loading ? "検索中…" : items.length ? "候補を選択" : "候補なし"}</option>{items.map((item) => <option key={item.id} value={item.id}>{item.displayName ?? item.name}{item.element ? `・${item.element}` : ""}{item.category ? `・${item.category}` : ""}</option>)}</select>{selectedId ? <button className="spark-master-clear" onClick={() => onSelect(null)} type="button">マスター選択を解除</button> : <small>候補がない場合は名前を直接入力できます。</small>}</fieldset>;
}
