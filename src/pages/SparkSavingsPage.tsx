...
function BalanceFields({ value, onChange }: { value: SparkSavingsInput; onChange: (value: SparkSavingsInput) => void }) {
  const normalize = (field: keyof SparkSavingsInput, raw: string) => onChange({ ...value, [field]: raw === "" ? "0" : raw });
  return <><label>宝晶石数<input inputMode="numeric" min="0" onBlur={(event) => normalize("crystalCount", event.target.value)} onChange={(e) => onChange({ ...value, crystalCount: e.target.value })} required type="number" value={value.crystalCount} /></label><label>単発チケット数<input inputMode="numeric" min="0" onBlur={(event) => normalize("singleTicketCount", event.target.value)} onChange={(e) => onChange({ ...value, singleTicketCount: e.target.value })} required type="number" value={value.singleTicketCount} /></label><label>10連チケット数<input inputMode="numeric" min="0" onBlur={(event) => normalize("tenPullTicketCount", event.target.value)} onChange={(e) => onChange({ ...value, tenPullTicketCount: e.target.value })} required type="number" value={value.tenPullTicketCount} /></label></>;
}
