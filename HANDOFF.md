...
基本版と拡張版を本番適用済み。拡張版は4タブ、狙い目と目標・編成リンク、任意履歴、共有データCRUD、監査ログ、3か月の獲得目安を含む。2026-08-05にpreflightで天井貯金1件、削除対象の使用予定日1件、目的・メモ0件を確認後、`20260805120000_expand_spark_savings`を適用した。残高（宝晶石21,361、単発20、10連2）の維持、新テーブル7種の空状態、全28 migration適用済みを確認した。Renderはコミット`7c05f7f`のビルドを配信し、health 200、画面200、新API未認証401、配信アセット一致を確認した。初期マスターは空であり、調査・ユーザー確認後に別途登録する。

後続の狙い目、排出時期、獲得目安、任意履歴は `docs/32_spark_savings_expansion_feature.md` に基づき実装・本番反映済み。排出時期と獲得イベントの初期マスターは未調査で、推測値を有効化しない。
