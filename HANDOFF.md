...
`SparkSavings`モデル、`20260805090000_add_spark_savings`、`/api/spark-savings`、`/spark-savings`、計算・入力検証テストを実装済み。migrationは新規テーブルと制約だけを追加する非破壊変更で、2026-08-05に本番DBへ適用した。Renderもコミット `f703cf6` のビルドを配信済みで、health check、画面ルート200、APIの未認証401、配信JSへの機能反映を確認した。認証済みユーザーによる保存・再取得・リセットと360px・768px・1280pxの実表示確認は未実施である。
