...
高難度攻略メモのマスタ本文はコード管理で、利用者は変更できない。対策メモは1利用者・1攻略メモにつき最大10件、付箋は1対策メモにつき最大20件で、変更・削除は所有者だけが行う。初期マスタはルシファー・ゼロ（33攻略行）。`20260804090000_add_raid_guides`は非破壊的migrationで、本番DBへの適用とseed実行は未実施。
